...
function kelpCompileCssFiles(inputExts, bundleType) {
  var folder = '';
  var filename = '';
  if (bundleType === 'library') {
    folder = 'lib';
    filename = '_kelp-library-bundle.scss';
  } else if (bundleType === 'css') {
    folder = 'styles';
    filename = 'kelp-css-bundle.scss';
  } else {
  ...
  var contents = '// THIS IS AN AUTOGENERATED KELP ' + bundleType.toUpperCase() + ' BUNDLE\n';
  if (bundleType === 'css') {
    contents += "@import 'kelp-library-bundle';\n\n";
  }
  extensions.forEach(function(ext) {
...
function libraryBundle(inputExts) {
  return kelpCompileCssFiles(inputExts, 'library');
}
...
function cssBundle(inputExts) {
  // return kelpCompileCssFiles(inputExts, 'css');
  // TODO: for some super weird reason, kelp/styles/_index.scss isn't showing up
  // BUG: We just need some way of waiting until all files finished before sending to sass
  return mergeStream(kelpCompileCssFiles(inputExts, 'library'), kelpCompileCssFiles(inputExts, 'css'))
